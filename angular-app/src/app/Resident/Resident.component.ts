...
export class ResidentComponent {
  ...
  carbonUnits = new FormControl("", Validators.required);
  ...
  loadAll(): Promise<any>  {
    ...
    .then(() => {
      ...
      //for each resident, get the associated carbon, coins, energy and cash asset
      for (let resident of residentList) {
        ...
        //get carbonID from the resident.carbon string
        var splitted_carbonID = resident.carbon.split("#", 2); 
        var carbonID = String(splitted_carbonID[1]);
        
        //call serviceResident to get energy asset
        this.serviceResident.getCarbon(carbonID)
        .toPromise()
        .then((result) => {
          this.errorMessage = null;
          //update resident
          if(result.value){
            resident.carbonValue = result.value;
          }
          if(result.units){
            resident.carbonUnits = result.units;
          }
        });
  ...
  createAssetsResident(): Promise<any> {
    ...
    //create carbon asset json
    this.carbon = {
      $class: "org.decentralized.energy.network.Carbon",
          "carbonID":"CC_" + this.residentID.value,
          "units":this.carbonUnits.value,
          "value":this.carbonValue.value,
    ...
    this.resident = {
      $class: "org.decentralized.energy.network.Resident",
          ...
          "carbon":"CC_" + this.residentID.value,
          "coins":"CO_" + this.residentID.value,
          "cash":"CA_" + this.residentID.value,
          "energy":"EN_" + this.residentID.value,
      };    

    //call serviceResident to add carbon asset, pass created carbon asset json as parameter
    return this.serviceResident.addCarbon(this.carbon)
    .toPromise()
		.then(() => {
  ...
  updateResident(form: any): Promise<any> {
    ...
    this.resident = {
      ...
      "firstName":this.firstName.value,          
      "lastName":this.lastName.value,
      "carbon": "resource:org.decentralized.energy.network.Carbon#CC_" + form.get("residentID").value,
  ...
  deleteResident(): Promise<any> {
    ...
    return this.serviceResident.deleteResident(this.currentId)
      ...
      this.serviceResident.deleteCarbon("CC_"+this.currentId)
		  .toPromise()
		  .then(() => {
